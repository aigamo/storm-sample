...
public class StormJunitExample extends TestCase {
		...
		Testing.withSimulatedTimeLocalCluster(mkClusterParam, new TestJob() {
			...
			public void run(ILocalCluster cluster) {
				...
				assertTrue(Testing.multiseteq(new Values(
						new Values("nathan", 1), new Values("nathan", 2),
						new Values("bob", 1), new Values("joey", 1)), Testing
						.readTuples(result, "2")));
				assertTrue(Testing.multiseteq(new Values(new Values(1),
						new Values(2), new Values(3), new Values(4)), Testing
						.readTuples(result, "3")));
				assertTrue(Testing.multiseteq(new Values(new Values(1),
						new Values(2), new Values(3), new Values(4)), Testing
						.readTuples(result, "4")));
	 ...
	 */
	/*
	 * public void testTestTuple() { // only specify values Tuple tuple =
	 * Testing.testTuple(new Values("james", "bond")); Testing.multiseteq(new
	 * Values("james", "bond"), tuple.getValues());
	 * 
	 * // specify stream, component and fields MkTupleParam param = new
	 * MkTupleParam(); param.setStream("test-stream");
	 * param.setComponent("test-component"); param.setFields("fname", "lname");
